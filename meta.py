...
# Extract META revenue data
META_rev = df["META_REVENUE"].tolist()

# Extract META earning data
earnings = df["META_EARNINGS"].tolist()

# Extract META dividends data
META_dividends = df["META_DIVIDENDS"].tolist()

# Remove the last data point
META_rev.pop(7)

# Convert to NumPy array
revenue = np.array(META_rev)
...
# Predict META revenue using power curve
y_power_pred = np.exp(power_model[1]) * (X_test.flatten() ** power_model[0])

# Visualize power curve fitting results
plt.scatter(X_test, y_test, color='blue', label='Actual META Revenue')
plt.plot(X_test, y_power_pred, color='red', label='Power Curve Fit')
plt.title('Power Curve Fitting: Actual vs Predicted META Revenue')
plt.xlabel('Quartiles')
plt.ylabel('META Revenue')
plt.legend()
