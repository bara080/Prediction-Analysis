...
# Extract IBM revenue data
ibm_rev = df["IBM_REVENUE"].tolist()

# Extract IBM earning data
earnings = df["IBM_EARNINGS"].tolist()

# Extract IBM dividends data
ibm_dividends = df["IBM_DIVIDENDS"].tolist()

# Remove the last data point
ibm_rev.pop(7)

# Convert to NumPy array
revenue = np.array(ibm_rev)
...
# Predict IBM revenue using power curve
y_power_pred = np.exp(power_model[1]) * (X_test.flatten() ** power_model[0])

# Visualize power curve fitting results
plt.scatter(X_test, y_test, color='blue', label='Actual IBM Revenue')
plt.plot(X_test, y_power_pred, color='red', label='Power Curve Fit')
plt.title('Power Curve Fitting: Actual vs Predicted IBM Revenue')
plt.xlabel('Quartiles')
plt.ylabel('IBM Revenue')
plt.legend()
