############################################
#
#  Data Science:  Prediction Model Analysis
#  Data Credit  : SEC Edgar
...
# Extract APPLE revenue data
APPLE_rev = df["APPLE_REVENUE"].tolist()

# Extract APPLE earning data
earnings = df["APPLE_EARNINGS"].tolist()

# Extract APPLE dividends data
APPLE_dividends = df["APPLE_DIVIDENDS"].tolist()

# Remove the last data point
APPLE_rev.pop(7)

# Convert to NumPy array
revenue = np.array(APPLE_rev)
...
# Predict APPLE revenue using power curve
y_power_pred = np.exp(power_model[1]) * (X_test.flatten() ** power_model[0])

# Visualize power curve fitting results
plt.scatter(X_test, y_test, color='blue', label='Actual APPLE Revenue')
plt.plot(X_test, y_power_pred, color='red', label='Power Curve Fit')
plt.title('Power Curve Fitting: Actual vs Predicted APPLE Revenue')
plt.xlabel('Quartiles')
plt.ylabel('APPLE Revenue')
plt.legend()
