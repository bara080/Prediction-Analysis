...
# Extract PG revenue data
PG_rev = df["PG_REVENUE"].tolist()

# Extract PG earning data
earnings = df["PG_EARNINGS"].tolist()

# Extract PG dividends data
PG_dividends = df["PG_DIVIDENDS"].tolist()

# Remove the last data point
PG_rev.pop(7)

# Convert to NumPy array
revenue = np.array(PG_rev)
...
# Predict PG revenue using power curve
y_power_pred = np.exp(power_model[1]) * (X_test.flatten() ** power_model[0])

# Visualize power curve fitting results
plt.scatter(X_test, y_test, color='blue', label='Actual PG Revenue')
plt.plot(X_test, y_power_pred, color='red', label='Power Curve Fit')
plt.title('Power Curve Fitting: Actual vs Predicted PG Revenue')
plt.xlabel('Quartiles')
plt.ylabel('PG Revenue')
plt.legend()
